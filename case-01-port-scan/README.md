...
File: evidence/case01-v7.pcap
Stats: 100,511 packets captured with zero drops.
Command used: sudo tcpdump -i ens33 -w /tmp/case01-v7.pcap host 192.168.10.10

<img width="664" height="68" alt="Screenshot 2026-08-29 151514" src="https://github.com/user-attachments/assets/0953ec04-f616-4af7-9c0a-bb3e3ab41a16" />


B. Attacker Perspective (Intent Confirmation)
From the Kali side, we confirmed the specific tool and parameters used.
File: evidence/nmap-output.png
Command used: sudo nmap -sS -Pn -p 1-100 --min-rate 10000 --script=default 192.168.20.10
...
<img width="850" height="90" alt="Screenshot 2026-08-29 151624" src="https://github.com/user-attachments/assets/581a2f77-9aa1-40cf-b6d2-03747d1684bb" />


C. IDS Alert Log (Real-time Detection)
Suricata logged the anomaly the moment it occurred. Note that the topmost entry in the log (Source 91.189.92.22) is unrelated background internet noise targeting port 80. Our focus is strictly on the correlated alerts from 192.168.10.10 below it, which show repeated SYN/ACK manipulation at timestamp 01:05:20.
File: evidence/suricata-alerts.png
Source: pfSense WebGUI > Services > Suricata > Alerts

<img width="626" height="332" alt="Screenshot 2026-08-29 151548" src="https://github.com/user-attachments/assets/12a568c8-c88c-42dd-86ca-e8f1ab9362cd" />
...
Immediate Block: Drop IP 192.168.10.10 at pfSense firewall (OPT1 inbound).
Host Hardening: Deploy Fail2Ban on Ubuntu to auto-ban IPs after 3-5 rapid connection attempts to port 22.
Detection Tuning: Add custom Suricata or Wazuh rules for high-rate SYN bursts to sensitive ports.
Log Review: Check /var/log/auth.log on Ubuntu to ensure no successful logins slipped through during the scan window.

## Detection Rule Reference
The signature logic behind this detection can be reviewed here:
detection/suricata-stream-anomaly.rules
